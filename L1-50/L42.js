...
//     for (let i = 1; i < len; i++) {
//         const stackHead = st[st.length - 1];
...
//                 let mid = st[st.length - 1]; // 将栈顶的值给缓存下来，然后进行出栈，这个值就是之前的栈顶值。原本栈顶值的左侧值就变成了现在的栈顶值
//                 st.pop();
//                 if (st.length !== 0) {
//                     // 使用 栈顶左侧的数值和准备入栈的数值 中的最小值来减去 之前栈顶的数值，求出容器的高度
    ...
    for (let i = 1; i < len; i++) {
        while(st.length > 0 && height[i] > height[st[st.length - 1]]) {
            ...
            if (st.length > 0) {
                ...
                // 这里计算出木板的最小高度，并且减去凹槽底部的高度就是能装水槽的高度
                const h = Math.min(height[i], height[st[st.length - 1]]) - height[mid];
                sum += w * h;
            }
        }
        st.push(i);
